...
Cron job to inference batch endpoint
"""
...
async def main():
    async with httpx.AsyncClient() as client:
        endpoint = "/api/inference/batch"
        for attempt in range(WORKER_RETRIES):
            try:
                response = await client.post(
                    f"{BACKEND_API_URL}{endpoint}",
                    headers=HEADERS,
                    json={"tier": ModelTier.CLOUD, "role": ModelRole.SCREENER},
            ...
            if response.status_code < 500 and response.status_code != 429:
                ...
                logger.info(
                    "dispatch_inference_batch_screener_completed",
                    endpoint=endpoint,
